...
int MapBuilderBridge::AddTrajectory(
  ...
  const int trajectory_id = map_builder_->AddTrajectoryBuilder(
      ...
      [this](const int trajectory_id, const ::cartographer::common::Time time,
        ...
        //
        cartographer::sensor::PointCloud newcloud;
        if(times < 1000){
          if(insertionresult.get() && insertionresult.get()->constant_data.get() && insertionresult.get()->constant_data.get()->filtered_gravity_aligned_point_cloud.size() != 0){
            
            times++;
            
            // std::string name = std::to_string(times) + ".pcd";
            // pcl::io::savePCDFileASCII (name,  *(insertionresult->constant_data->submap_pointcloud));
            if(times %  2 == 1){
              cartographer::sensor::PointCloud range = insertionresult.get()->constant_data.get()->filtered_gravity_aligned_point_cloud;
              
              for(size_t i = 0; i < range.points().size(); i++){
                cartographer::sensor::PointCloud::PointType point;
                //恢复重力修正前
                point.position = insertionresult.get()->constant_data.get()->gravity_alignment.matrix().inverse().cast<float>() * range.points().at(i).position;
                newcloud.push_back(point);
              }
         
              auto pcl_cloud = cartographer_ros::ToPointCloudMessage(newcloud);
              std::string name = std::to_string(times) + ".pcd";
              pcl::io::savePCDFileASCII (name, pcl_cloud);
            }
          }
        }
