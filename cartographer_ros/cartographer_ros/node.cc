...
Node::Node(
  ...
  // cloudpoint_list_publisher_ = 
  //     node_handle_.advertise<sensor_msgs::PointCloud>(
  //         "cloudpoint", kLatestOnlyPublisherQueueSize);
...
int Node::AddTrajectory(const TrajectoryOptions& options) {
  ...
  // 调用map_builder_bridge的AddTrajectory, 添加一条轨迹
...
void Node::LaunchSubscribers(const TrajectoryOptions& options,
  ...
  for (const std::string& topic :
    ...
    subscribers_[trajectory_id].push_back(
        {
          SubscribeWithHandler<sensor_msgs::LaserScan>(//返回一个ros
